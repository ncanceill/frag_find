...
static inline void set_bloom_bit(nsrl_bloom *b,uint64_t bit)
{
 ...
 */
static inline int get_bloom_bit(const nsrl_bloom *b,uint64_t bit)
{
...
void nsrl_bloom_fprint_info(const nsrl_bloom *b,FILE *out)
{
    ...
    fprintf(out,"added_items: %"PRIu64"\n",b->added_items);
    fprintf(out,"unique_added_items: %"PRIu64"\n",b->unique_added_items);
    fprintf(out,"aliased_adds: %"PRIu64"\n",b->aliased_adds);
...
void nsrl_bloom_info(char *buf,size_t buflen, const nsrl_bloom *b)
{
    snprintf(buf,buflen,
	     ...
	     "k:%"PRIu32"\n"
	     "added_items:%"PRIu64"\n"
	     "unique_added_items:%"PRIu64"\n"
	     "aliased_adds:%"PRIu64"\n"
	     "comment:%s\n",
	     (b->hash_bytes*8), b->M,b->k,
	     b->added_items,b->unique_added_items,b->aliased_adds,b->comment);
}
...
{
    size_t i;
    memset(counts,0,256*sizeof(uint64_t)); /* clear the counter */
...
void nsrl_print_histogram(const nsrl_bloom *b,const uint64_t counts[256])
{
    ...
    totalbits = (uint64_t)0x1l << b->M;
    printf("%"PRIu64" bits total, %"PRIu64" bits set\n",totalbits, bitcount);
    top      = (float)bitcount;
...
void nsrl_bloom_add(nsrl_bloom *b,const unsigned char *hash)
{
	...
	if(b->debug>1) { printf(" Setting bit: %08"PRIu64"x (was %d).\n",v,get_bloom_bit(b,v)); }
    ...
    if(added_bits==0)    b->aliased_adds++;       /* none of the bits were set */
    b->added_items++;
...
int nsrl_bloom_query_check( nsrl_bloom *b,const unsigned char *hash, int just_checking)
{
	...
	if(b->debug>1){ printf("  %s V(0x%08"PRIu64"x)=%d\n", (i==0?"VECTORS":"       "), v,get_bloom_bit(b,v)); }
	if(get_bloom_bit(b,v)==0){
	    found = 0;
	}
    }
...
double nsrl_bloom_utilization(const nsrl_bloom *b)
{
    uint64_t bits_set  = 0;
    int i,j,mask;
    uint8_t bits_per[256];

    // pre calculate bits needed for each byte
    // this could be packed into a constant or macro:
    for (i = 0; i < 256; ++i) {
       bits_per[i] = 0;
         for(j = 0; j<8; ++j){
           mask = (0x1 << j);
...
int nsrl_bloom_create(nsrl_bloom *b,
		      ...
		      const char *comment)
{
    b->vector_bytes = (size_t)0x1l << (bloom_bits-3);	/* Needed vector size */
